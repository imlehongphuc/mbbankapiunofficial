...
(() => {
  ...
  if (!globalThis.fs) {
    ...
    globalThis.fs = {
      ...
      write(fd, buffer, offset, length, position, callback) {
        if (0 === offset && length === buffer.length && null === position) {
      ...
      fsync(fd, callback) {
        callback(null); // tại sao lại là null mà k phải là ENOSYS ?
      },
    };
  }
  ...
  if (!globalThis.crypto) { // Cái này là WebCrypto != NodeJS Crypto thì phải ;-;
    throw new Error(
  ...
  globalThis.Go = class {
    constructor() {
      ...
      this.importObject = {
        _gotest: {
          add: (a, b) => a + b, // thực sự mấy ông dev MBBank bê nguyên cái example của TinyGo về à ;-;
        },
        gojs: {
          ...
          "syscall/js.finalizeRef": (_0x5edca4) => {
            ...
            if (0x0 === this._goRefCounts[_0x57d4fc]) {
...
export default function (wasmBytes, requestData, args1) {
  return processAsync(this, null, function* () {
    const go = new Go();
    let instance = (yield WebAssembly.instantiate(wasmBytes, go.importObject))
      .instance;
    go.run(instance);
